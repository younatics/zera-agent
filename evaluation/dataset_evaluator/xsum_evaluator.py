...
class XSUMEvaluator(BaseEvaluator):
    ...
    def load_dataset(self, dataset_path: str = "xsum", num_samples: Optional[int] = None) -> List[Dict[str, Any]]:
        """Load XSUM dataset. Sample file (json) management method."""
        if num_samples:
    ...
    def format_question(self, item: Dict[str, Any]) -> str:
        """Use XSUM article (document) as input."""
        return item['document']

    def evaluate_response(self, response: str, ground_truth: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluate XSUM summary (always treat as correct without ROUGE-L criteria)."""
        try:
    ...
    def run_evaluation(self, 
        ...
        """Method to execute the entire evaluation (same method as CNN/DailyMail evaluator)."""
